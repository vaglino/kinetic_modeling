...
function run_model(model,p,data,densities,constants,t) # version of run function with multiple models
    ...
    f = (du,u,p,t) -> model(du,u,p,t,densities,constants)
...
function loss(p,data,model,dens,cons,t,unique_t)
    ...
    for i in 1:length(data)
      ...
      tmp_sol = run_model(model,p,data[i],dens[i],cons[i],unique_t[i])

      if tmp_sol.retcode != :Success #if there is an error return infitite loss
        Δy_sol = fill(Inf, length(data[i]))
      else
        Δy_sol = find_diffs(tmp_sol,t[i],unique_t[i],data[i])
      end
...
# fit function - given model, data, densities, options get best parameters
  unique_t = [unique(times) for times in t]
  loss_in = (p) -> loss(p,data,model,dens,cons,t,unique_t)
  ...
  # hes_for = ReverseDiff.hessian(loss_in,rates)
  # # hes_for = ForwardDiff.hessian(loss_in,rates)
  # hes_zyg = Zygote.hessian(loss_in,rates)
  # @show hes_for ≈ hes_zyg
  hes_zyg = []
  return rates , hes_zyg, l
end
